...
public:
    ...
    /// Export a single BSplines patch in 1D to geometry file that can be read by geo_load routine.
    ...
    template<int TDim>
    void ExportGlvis(typename MultiPatch<TDim>::Pointer pMultiPatch, const std::string& filename) const
    {
        std::ofstream outfile;
        outfile.open(filename, std::ios::out);
        ...
        this->GenerateCornerTopology<TDim>(*pMultiPatch, nvertices, elements, boundary, edges, knotvec);
