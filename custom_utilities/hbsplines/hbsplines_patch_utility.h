...
public:
    ...
    static typename Patch<TDim>::Pointer CreatePatchFromBSplines(typename Patch<TDim>::Pointer pPatch)
    {
        ...
        typename HBSplinesFESpace<TDim>::Pointer pNewFESpace = HBSplinesFESpace<TDim>::Create();

        for (int dim = 0; dim < TDim; ++dim)
            pNewFESpace->SetInfo(dim, pPatch->pFESpace()->Order(dim));

        typedef typename BSplinesFESpace<TDim>::knot_t knot_t;
        ...
        if(TDim == 2)
        {
            ...
            for(std::size_t j = 0; j < number_2; ++j)
            {
                ...
                for(std::size_t i = 0; i < number_1; ++i)
                {
                    ...
                    typename HBSplinesBasisFunction<TDim>::Pointer p_bf = pNewFESpace->CreateBf(id, level, pLocalKnots);
        ...
        typename Patch<TDim>::Pointer pNewPatch = Patch<TDim>::Create(pPatch->Id(), pNewFESpace);

        // set control points grid
        typename ControlGrid<ControlPointType>::Pointer pControlPointGrid
            = ControlGridUtility::CreatePointBasedControlGrid<ControlPointType, HBSplinesFESpace<TDim> >(CONTROL_POINT, pNewFESpace);
